...
def test_get_financial_summary(client, test_financial_summary):
    """Test getting a financial summary by ID."""
    response = client.get(f"/financial-summaries/{test_financial_summary.id}")
    assert response.status_code == 200
    data = response.json()
...
def test_get_nonexistent_financial_summary(client):
    """Test getting a financial summary that doesn't exist."""
    response = client.get("/financial-summaries/999")
    assert response.status_code == 404
    assert "FinancialSummary not found" in response.json()["detail"]

def test_update_financial_summary(client, test_financial_summary):
    """Test updating a financial summary."""
    response = client.put(
        f"/financial-summaries/{test_financial_summary.id}",
        json={
...
def test_update_nonexistent_financial_summary(client):
    """Test updating a financial summary that doesn't exist."""
    response = client.put(
        "/financial-summaries/999",
        json={
...
def test_delete_financial_summary(client, test_financial_summary):
    """Test deleting a financial summary."""
    response = client.delete(f"/financial-summaries/{test_financial_summary.id}")
    assert response.status_code == 200
    data = response.json()
    assert data["id"] == test_financial_summary.id
...
def test_delete_nonexistent_financial_summary(client):
    """Test deleting a financial summary that doesn't exist."""
    response = client.delete("/financial-summaries/999")
    assert response.status_code == 404
    assert "FinancialSummary not found" in response.json()["detail"]
